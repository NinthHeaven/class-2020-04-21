...
female_trump <- cces %>%
  ...
  summarize(mean_vote = mean(trump == 1))
```
...
# Females are ~8% less likely to vote for trump taking into account the other variables.

# Other strategy: Create the tibble 'dom' with the features printed below.
...
# Dom would fall on the left-hand side of the classification tree because his party value is less than 4 so he is more likely to not vote for Trump.

set.seed(1005)

forest <- rand_forest(mtry = 7, trees = 50) %>%
  set_engine("randomForest") %>%
  set_mode("classification")

cces_forest <- fit(forest,
                   trump ~ gender + age + educ + race + income + party + ideology,
                  data = cces)
